...
for (let x = 0; x < data.length; x++) {
	for(let y = 0; y < data[0].length; y++) {
		...
		if (x > 0) {
			...
			if ((current.height - 1) <= up.height) current.neighbours.push(up);
		}
		if (x < data.length - 1) {
			const down = g.getVertex(x + 1, y);
			if ((current.height - 1) <= down.height) current.neighbours.push(down);
		}
		if (y > 0) {
			const left = g.getVertex(x, y - 1);
			if ((current.height - 1) <= left.height) current.neighbours.push(left);
		}
		if (y < data[0].length - 1) {
			const right = g.getVertex(x, y + 1);
			if ((current.height - 1) <= right.height) current.neighbours.push(right);
		}
	}
}
...
while (queue.length !== 0) {
	u = queue.sort((a, b) => a.dist > b.dist ? -1 : 1).pop();
...
for (let x = 0; x < data.length; x++) {
	for(let y = 0; y < data[0].length; y++) {
		if (g.getVertex(x, y).height === 1 && g.getVertex(x, y).dist < min) {
			min = g.getVertex(x, y).dist;
		}
	}
}
