...
class Monkey {
	...
	inspectItem(index, range) {
		const item = this.items[index];

		// 1. Compute new worry level after inspection
		let newWorryLevel = eval(this.operation.replaceAll('old', item.worryLevel));
		// 2. Keep worry at a managable level by keeping it in range of the product of each divider
		newWorryLevel = newWorryLevel % range;
...
const worriesRange = monkeys.reduce((acc, curr) => acc * curr.divider, 1);
...
for (let round = 0; round < 10000; round++) {
	monkeys.forEach(monkey => {
		monkey.items.forEach((item, i) => {
			monkey.inspectItem(i, worriesRange);
